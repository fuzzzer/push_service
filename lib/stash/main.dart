// ignore_for_file: prefer_single_quotes, deprecated_member_use
...
import 'package:push_service/stash/history_screen.dart';
import 'package:shared_preferences/shared_preferences.dart';

import 'fcm_history_entry_old.dart'; // Import the package
...
Future<void> runner() async {
  ...
  Hive.registerAdapter(FcmHistoryEntryOldAdapter());
  await Hive.openBox<FcmHistoryEntryOld>(historyBoxName);
    ...
    return ValueListenableBuilder<ThemeMode>(
      ...
      builder: (_, mode, __) {
        return MaterialApp(
          ...
          home: OldFcmSenderScreen(themeNotifier: themeNotifier), // Pass notifier down
        );
      },
    );
  }
...
class OldFcmSenderScreen extends StatefulWidget {
  final ValueNotifier<ThemeMode> themeNotifier; // Accept notifier

  const OldFcmSenderScreen({super.key, required this.themeNotifier}); // Update constructor

  @override
  State<OldFcmSenderScreen> createState() => _OldFcmSenderScreenState();
}

class _OldFcmSenderScreenState extends State<OldFcmSenderScreen> {
  // --- State Variables ---
  ...
  Future<void> _saveToHistory({
    ...
    try {
      await Hive.openBox<FcmHistoryEntryOld>(historyBoxName); // Open the history box
      final historyBox = Hive.box<FcmHistoryEntryOld>(historyBoxName);
      final entry = FcmHistoryEntryOld(
  ...
  Future<String?> _getAccessToken(String serviceAccountJson) async {
    ...
    try {
      ...
      _addToLog('Access token obtained successfully.: ${accessCredentials.accessToken.data}');
